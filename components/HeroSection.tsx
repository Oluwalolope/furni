...
const HeroSection = ({title, description}: Props) => {
    return (
        <div className="bg-background-header pt-[132px]">
            <div className="flex flex-col justify-start w-full max-w-[1512px] mx-auto px-4 md:px-6 lg:px-[138px] pb-44 relative">
                ...
                { (title != 'shop' && title != 'cart' )  && <div>
                    ...
                    { (title != 'shop' && title != 'cart' ) && 
                        <>
                            <Image src={heroBackgroundPattern} alt="Hero Background pattern" aria-hidden  className='scale-[0.7] absolute right-8 bottom-35 opacity-30 hidden lg:block' draggable={false}/>
                            <Image src={heroBackground} alt="Hero Background" aria-hidden  className='scale-[0.7] absolute -right-32  hidden lg:block' draggable={false}/>
                        </>
                    }
            </div>
        </div>
