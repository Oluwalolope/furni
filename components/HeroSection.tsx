...
const HeroSection = ({title, description}: Props) => {
    return (
        <div className="bg-background-header pt-[132px]">            <div className="flex flex-col justify-start w-full max-w-[1512px] mx-auto px-4 md:px-6 lg:px-[138px] pb-44 relative">
                <h1 className="capitalize font-bold font-inter text-[3rem] max-w-[520px] text-white">{title}</h1>
                { (title != 'shop' && title != 'cart' )  && <div>
                    <p className="pt-7 pb-9 max-w-[520px] text-[#94A7A0] font-inter font-medium">{description}</p>
                    <div className="flex flex-row gap-2.5">
                        <Link href='/shop' className="py-3 px-8 bg-[#F9BF29] hover:bg-[#FFD057] transition-all duration-300 cursor-pointer rounded-[100px] capitalize text-[#2F2F2F] font-inter font-semibold  text-lg" >shop now</Link>

                        <Link href='/services' className="py-3 px-8 border border-[#94A7A0] hover:border-white transition-all duration-300 cursor-pointer rounded-[100px] text-white font-inter font-semibold text-lg capitalize" >Explore</Link>
                    </div>
