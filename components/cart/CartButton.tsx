...
const CartButton = () => {
    ...
    if (path === '/cart' || path === '/cart/checkout') {
    ...
    return (
        <Link href='/cart' className="relative">
            ...
            {cartCtx.cartItems.length !== 0 && <div className="size-5 bg-amber-600 font-inter font-medium text-white text-[14px] rounded-full absolute -top-1 -right-2 grid place-items-center">{cartCtx.cartItems.length}</div>}
        </Link>
    );
}
